...
# commands into the Float64 commands required by the Gazebo wheel and steering
# controllers.
# -------------------------------------------------------------------------
...
class BlueAckermannLowLevelBridge(object):
    ...
    def __init__(self):
        ...
        # wheel_speed_sign is 1.0 when a positive wheel command moves the car
        # forward. steering_angle_sign can be changed if turns are inverted.
        # -------------------------------------------------------------------------
        ...
        # These are the Gazebo controller topics for the BLUE wheel and steering
        # controllers.
        # -------------------------------------------------------------------------
        ...
        # The planner publishes AckermannDrive messages here. This bridge makes
        # /blue/ackermann_cmd actually reach the low-level Gazebo controllers.
        # -------------------------------------------------------------------------
    ...
    def clamp(self, value, min_value, max_value):
        # -------------------------------------------------------------------------
        # BLOCK 5: Numeric saturation helper.
        # It protects the low-level controllers from excessive commands.
        # -------------------------------------------------------------------------
        return max(min_value, min(max_value, value))

    def ackermann_callback(self, command):
        # -------------------------------------------------------------------------
        # BLOCK 6: Command conversion.
    ...
    def stop_robot(self):
        # -------------------------------------------------------------------------
        # BLOCK 8: Safety stop.
        # Sends zero velocity and zero steering when the node finishes or when the
        # planner stops publishing commands.
        # -------------------------------------------------------------------------
        self.publish_low_level_command(0.0, 0.0)

    def run(self):
        # -------------------------------------------------------------------------
        # BLOCK 9: Watchdog loop.
